...
public class CmdTransports extends CmdBase {
    ...
    public CmdTransports(Supplier<PulsarAdmin> admin) {
    ...
    @Parameters(commandDescription = "Run a Pulsar IO transport connector locally (rather than deploying it to the "
            + "Pulsar cluster)")
    protected class LocalTransportRunner extends CreateTransport {
        ...
        @Parameter(names = "--client-auth-plugin", description = "Client authentication plugin using which "
        ...
        @Parameter(names = "--hostname_verification_enabled", description = "Enable hostname verification", hidden =
                true)
    ...
    protected class CreateTransport extends TransportDetailsCommand {
        ...
        void runCmd() throws Exception {
            if (Utils.isFunctionPackageUrlSupported(archive)) {
                getAdmin().transports().createTransportWithUrl(transportConfig, transportConfig.getArchive());
            } else {
                getAdmin().transports().createTransport(transportConfig, transportConfig.getArchive());
            }
    ...
    protected class UpdateTransport extends TransportDetailsCommand {
        ...
        void runCmd() throws Exception {
            ...
            if (Utils.isFunctionPackageUrlSupported(archive)) {
                getAdmin().transports()
                        .updateTransportWithUrl(transportConfig, transportConfig.getArchive(), updateOptions);
            } else {
                getAdmin().transports().updateTransport(transportConfig, transportConfig.getArchive(), updateOptions);
            }
    ...
    abstract class TransportDetailsCommand extends BaseCommand {
        ...
        @Parameter(names = "--logTopic", description = "The topic to which the logs of a Pulsar Function are "
                + "produced", hidden = true)
        ...
        @Parameter(names = "--processingGuarantees", description = "The processing guarantees (aka delivery "
                + "semantics) applied to the transport", hidden = true)
        protected FunctionConfig.ProcessingGuarantees DEPRECATED_processingGuarantees;
        @Parameter(names = "--processing-guarantees", description = "The processing guarantees (aka delivery "
                + "semantics) applied to the transport")
        protected FunctionConfig.ProcessingGuarantees processingGuarantees;
        @Parameter(names = "--retainOrdering", description = "Transport consumes and transports messages in order",
                hidden = true)
        ...
        @Parameter(names = "--parallelism", description = "The transport's parallelism factor (i.e. the number of "
                + "transport instances to run)")
        protected Integer parallelism;
        @Parameter(names = {"-a",
                "--archive"}, description = "Path to the archive file for the transport. It also supports url-path "
                + "[http/https/file (file protocol assumes that file already exists on worker host)] from which "
                + "worker can download the package.", listConverter = StringConverter.class)
        protected String archive;

        @Parameter(names = "--sinkClassName", description = "The transport's class name if archive is file-url-path "
                + "(file://)")
        protected String sinkClassName;

        @Parameter(names = "--sourceClassName", description = "The transport's class name if archive is file-url-path"
                + " (file://)")
        protected String sourceClassName;

        @Parameter(names = "--functionClassName", description = "The transport's class name if archive is "
                + "file-url-path (file://)")
        ...
        @Parameter(names = "--cpu", description = "The CPU (in cores) that needs to be allocated per transport "
                + "instance (applicable only to Docker runtime)")
        protected Double cpu;
        @Parameter(names = "--ram", description = "The RAM (in bytes) that need to be allocated per transport "
                + "instance (applicable only to the process and Docker runtimes)")
        protected Long ram;
        @Parameter(names = "--disk", description = "The disk (in bytes) that need to be allocated per transport "
                + "instance (applicable only to Docker runtime)")
        protected Long disk;
        ...
        @Parameter(names = "--auto-ack", description = "Whether or not the framework will automatically acknowledge "
                + "messages", arity = 1)
        protected Boolean autoAck;
        @Parameter(names = "--custom-runtime-options", description = "A string that encodes options to customize the "
            ...
            if (!Utils.isFunctionPackageUrlSupported(transportConfig.getArchive()) &&
                ...
                if (!new File(transportConfig.getArchive()).exists()) {
                    throw new IllegalArgumentException(
                            String.format("Transport Archive file %s does not exist", transportConfig.getArchive()));
                }
            }
        }
        ...
        protected String validateTransportType(String transportType) throws IOException {
            ...
            try {
                availableTransports = getAdmin().transports().getBuiltInTransports().stream()
            ...
            if (!availableTransports.contains(transportType)) {
                throw new ParameterException(
                        "Invalid transport type '" + transportType + "' -- Available transports are: "
                                + availableTransports);
            }
    ...
    protected class DeleteTransport extends TransportCommand {
        ...
        @Override
        void runCmd() throws Exception {
            getAdmin().transports().deleteTransport(tenant, namespace, transportName);
    ...
    protected class GetTransport extends TransportCommand {
        ...
        @Override
        void runCmd() throws Exception {
            TransportConfig transportConfig = getAdmin().transports().getTransport(tenant, namespace, transportName);
    ...
    protected class ListTransports extends BaseCommand {
        ...
        @Override
        void runCmd() throws Exception {
            List<String> transports = getAdmin().transports().listTransports(tenant, namespace);
            Gson gson = new GsonBuilder().setPrettyPrinting().create();
    ...
    class GetTransportStatus extends TransportCommand {
        ...
        @Parameter(names = "--instance-id", description = "The transport instanceId (Get-status of all instances if "
        ...
        void runCmd() throws Exception {
            if (isBlank(instanceId)) {
                print(getAdmin().transports().getTransportStatus(tenant, namespace, transportName));
            } else {
                print(getAdmin().transports()
                        .getTransportStatus(tenant, namespace, transportName, Integer.parseInt(instanceId)));
            }
        }
    }
    ...
    class RestartTransport extends TransportCommand {
        ...
        @Parameter(names = "--instance-id", description = "The transport instanceId (restart all instances if "
                + "instance-id is not provided")
        protected String instanceId;
        ...
        void runCmd() throws Exception {
            if (isNotBlank(instanceId)) {
                try {
                    getAdmin().transports()
                            .restartTransport(tenant, namespace, transportName, Integer.parseInt(instanceId));
                } catch (NumberFormatException e) {
                    System.err.println("instance-id must be a number");
                }
            } else {
                getAdmin().transports().restartTransport(tenant, namespace, transportName);
            }
    ...
    class StopTransport extends TransportCommand {
        ...
        @Parameter(names = "--instance-id", description = "The transport instanceId (stop all instances if "
                + "instance-id is not provided")
        protected String instanceId;
        ...
        void runCmd() throws Exception {
            if (isNotBlank(instanceId)) {
                try {
                    getAdmin().transports()
                            .stopTransport(tenant, namespace, transportName, Integer.parseInt(instanceId));
                } catch (NumberFormatException e) {
                    System.err.println("instance-id must be a number");
                }
            } else {
                getAdmin().transports().stopTransport(tenant, namespace, transportName);
            }
    ...
    class StartTransport extends TransportCommand {
        ...
        @Parameter(names = "--instance-id", description = "The transport instanceId (start all instances if "
                + "instance-id is not provided")
        protected String instanceId;
        ...
        void runCmd() throws Exception {
            if (isNotBlank(instanceId)) {
                try {
                    getAdmin().transports()
                            .startTransport(tenant, namespace, transportName, Integer.parseInt(instanceId));
                } catch (NumberFormatException e) {
                    System.err.println("instance-id must be a number");
                }
            } else {
                getAdmin().transports().startTransport(tenant, namespace, transportName);
            }
    ...
    public class ListBuiltInTransports extends BaseCommand {
        ...
        @Override
        void runCmd() throws Exception {
            getAdmin().transports().getBuiltInTransports().stream()
                    .filter(x -> isNotBlank(x.getSinkClass()) && isNotBlank(x.getSourceClass()))
                    .forEach(connector -> {
    ...
    public class ReloadBuiltInTransports extends BaseCommand {
        ...
        @Override
        void runCmd() throws Exception {
            getAdmin().transports().reloadBuiltInTransports();
        }
    }
