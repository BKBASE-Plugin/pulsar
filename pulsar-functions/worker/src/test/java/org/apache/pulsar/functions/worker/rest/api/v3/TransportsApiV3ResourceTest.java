 ...
 */
@PrepareForTest({WorkerUtils.class, TransportConfigUtils.class, ConnectorUtils.class, FunctionCommon.class,
...
@PowerMockIgnore({"javax.management.*", "javax.ws.*", "org.apache.logging.log4j.*", "org.apache.pulsar.io.*",
        "java.io.*"})
public class TransportsApiV3ResourceTest {
    ...
    public void setup() throws Exception {
        ...
        INVALID_JAR_FILE_PATH = Thread.currentThread().getContextClassLoader().getResource(INVALID_JAR_FILE_NAME)
                .getFile();
    ...
    @Test(expectedExceptions = RestException.class, expectedExceptionsMessageRegExp = "Transport package is not "
            + "provided")
    ...
    @Test(expectedExceptions = RestException.class, expectedExceptionsMessageRegExp = "Encountered error .*. when "
            + "getting Transport package from .*")
    ...
    @Test(expectedExceptions = RestException.class, expectedExceptionsMessageRegExp = "Transport config is not "
            + "provided")
    ...
    @Test(expectedExceptions = RestException.class, expectedExceptionsMessageRegExp = "Transport test-source already "
            + "exists")
    ...
    public void testRegisterTransportConflictingFields() throws Exception {
        ...
        transportConfig.setFunctionClassName(functionClassName);
        transportConfig.setParallelism(parallelism);
        transportConfig.setSinkClassName(CASSANDRA_STRING_SINK);
        transportConfig.setSourceClassName(TWITTER_FIRE_HOSE);
        transport.registerTransport(
                actualTenant,
                actualNamespace,
                actualName,
    ...
    public void testRegisterTransportFailure() throws Exception {
        try {
            ...
            doThrow(new IllegalArgumentException("transport failed to register"))
    ...
    @Test(expectedExceptions = RestException.class, expectedExceptionsMessageRegExp = "java.io.IOException: Function "
    ...
    @Test(expectedExceptions = RestException.class, expectedExceptionsMessageRegExp = "Transport parallelism must be "
            + "a positive number")
    ...
    private void testUpdateTransportMissingArguments(
        ...
        when(FunctionCommon.convertProcessingGuarantee(FunctionConfig.ProcessingGuarantees.ATLEAST_ONCE))
                .thenReturn(ProcessingGuarantees.ATLEAST_ONCE);
        doReturn(mock(NarClassLoader.class)).when(FunctionCommon.class);
        FunctionCommon.extractNarClassLoader(any(), any());

        this.mockedFunctionMetaData = FunctionMetaData.newBuilder().setFunctionDetails(createDefaultFunctionDetails())
                .build();
    ...
    private void updateDefaultSource() throws Exception {
        ...
        sourceConfig.setSinkClassName(CASSANDRA_STRING_SINK);
        sourceConfig.setSourceClassName(TWITTER_FIRE_HOSE);
    ...
    @Test(expectedExceptions = RestException.class, expectedExceptionsMessageRegExp = "Transport test-source doesn't "
            + "exist")
    ...
    public void testDeregisterSourceSuccess() {
        ...
        when(mockedManager.getFunctionMetaData(eq(tenant), eq(namespace), eq(transportName)))
    ...
    public void testOnlyGetTransports() {
        ...
        PowerMockito.when(InstanceUtils.calculateSubjectType((f1.getFunctionDetails())))
                .thenReturn(Function.FunctionDetails.ComponentType.SOURCE);
        PowerMockito.when(InstanceUtils.calculateSubjectType((f2.getFunctionDetails())))
                .thenReturn(Function.FunctionDetails.ComponentType.FUNCTION);
        PowerMockito.when(InstanceUtils.calculateSubjectType((f3.getFunctionDetails())))
                .thenReturn(Function.FunctionDetails.ComponentType.SINK);
        PowerMockito.when(InstanceUtils.calculateSubjectType((f4.getFunctionDetails())))
                .thenReturn(Function.FunctionDetails.ComponentType.TRANSPORT);
    ...
    private FunctionDetails createDefaultFunctionDetails() {
        return TransportConfigUtils.convert(createDefaultTransportConfig(),
                new TransportConfigUtils.ExtractedTransportDetails(null, null, null, null, null));
    }
